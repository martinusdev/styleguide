...
- Regex `new RegExp('^mixin\\s+${name}\\s*(\\([^)]*\\))?([\\s\\S]*?)(?=^mixin\\s|$)', 'm')` — lookahead `(?=^mixin\s|$)` **nemá flag `m` v lookaheade** (regex flag platí pre celý pattern, ale `^` v lookahead pri globálnom `m` by mal fungovať — problém je však, že `new RegExp(..., 'm')` sa aplikuje, ALE chybe sme overili manuálnym tracovaním).
- **Praktický dopad:** `views/mixins/product.pug` obsahuje 10 mixinov. Požiadavka na `productCover` vráti telo od `productCover` až po koniec súboru (všetkých 10 mixinov spolu). Rovnako `booksCarouselRecommended.pug` (3 mixiny), `collectionItem.pug` (2), `icon.pug` (2).
- **Oprava:** parsovať Pug mixiny podľa indentácie, nie podľa regexu na „ďalší `mixin `".
